...
exports.addTracingMiddleware = (tracer) => {
    const concurrentRequests = metrics_1.createConcurrentRequestsInstrument();
    const requestTimings = metrics_1.createRequestsTimingsInstrument();
    const totalRequests = metrics_1.createTotalRequestsInstrument();
    const responseSizes = metrics_1.createRequestsResponseSizesInstrument();
    const abortedRequests = metrics_1.createTotalAbortedRequestsInstrument();
    return async function addTracing(ctx, next) {
        ...
        ctx.req.once('abort', () => abortedRequests.inc({ ["handler" /* REQUEST_HANDLER */]: currentSpan.operationName }, 1));
        try {
            await next();
        }
        catch (err) {
